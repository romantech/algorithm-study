 ...
 * 이처럼 n개 원판에 대한 문제를 해결하려면 두 번의 n-1개 원판 문제를 풀고 추가로 1번의 이동을 수행해야 한다
 * 따라서 시간 복잡도는 영향력 없는 항을 무시해서 O(2ⁿ) 지수시간
 */
...
// n === 3 시뮬레이션
// =================
// h(3, 1, 3, 2, []) 3개 원판의 하노이 탑 문제 해결
// └─ (n3 전) h(2, 1, 2, 3) 보조 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
//   └─ (n2 전) h(1, 1, 3, 2) => [[1, 3]]
//   └─ (n2 중) [[1, 3], [1, 2]]
//   └─ (n2 후) h(1, 3, 2, 1) => [[1, 3], [1, 2], [3, 2]]
// └─ (n3 중) [[1, 3], [1, 2], [3, 2], [1, 3]]
// └─ (n3 후) h(2, 2, 3, 1) 목표 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
...
// n === 4 시뮬레이션
// ================
// h(4, 1, 3, 2, []) 4개 원판의 하노이 탑 문제 해결
// └─ (n4 전) h(3, 1, 2, 3) 보조 기둥으로 옮기기 위해 3개 원판의 하노이 탑 문제 해결
//   └─ (n3 전) h(2, 1, 3, 2) 보조 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
//     └─ (n2 전) h(1, 1, 2, 3) => [[1, 2]]
//     └─ (n2 중) [[1, 2], [1, 3]]
//     └─ (n2 후) h(1, 2, 3, 1) => [[1, 2], [1, 3], [2, 3]]
//   └─ (n3 중) [[1, 2], [1, 3], [2, 3], [1, 2]]
//   └─ (n3 후) h(2, 3, 2, 1) 목표 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
...
// └─ (n4 후) h(3, 2, 3, 1) 목표 기둥으로 옮기기 위해 3개 원판의 하노이 탑 문제 해결
//   └─ (n3 전) h(2, 2, 1, 3) 보조 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
...
//   └─ (n3 후) h(2, 1, 3, 2) 목표 기둥으로 옮기기 위해 2개 원판의 하노이 탑 문제 해결
